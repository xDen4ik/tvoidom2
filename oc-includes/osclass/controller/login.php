...
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/ApiInterface.php");
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/ApiClient.php");
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/Storage/TokenStorageInterface.php");
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/Storage/FileStorage.php");
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/Storage/SessionStorage.php");
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/Storage/MemcachedStorage.php");
require_once($_SERVER["DOCUMENT_ROOT"] . "/oc-includes/sendpulse/src/Storage/MemcacheStorage.php");
    ...
    function doModel()
    {
        switch ($this->action) {
            case ('login_post'): //post execution for the login
                $number = intval(Params::getParam('s_phone_mobile'));
                if (strlen($number) > 9 && is_numeric($number)) {
                    $code_auth = rand(100000, 999999);
                    /* $sms = new ApiClient('237034b7b2e27471a74c327d1c3c6fc1', 'a528f9e62a07d2063f1b7279aa19f167', new FileStorage());
                    $phones = [$number];
                    $params = [
                        'sender' => 'TvoyDom',
                        'type' => 'string',
                        'body' => 'Ваш код: ' . $code_auth,
                    ];
                    $additionalParams = [
                        'transliterate' => 0
                    ];
                    $sms->sendSmsByList($phones, $params, $additionalParams); */
                }
                $userManager = new User();
                $userManager->update(
                    array('s_phone_land' => $code_auth),
                    array('s_phone_mobile' => Params::getParam('s_phone_mobile'))
                );
                return false;
            ...
            default: //login
                Session::newInstance()
                    ->_setReferer(osc_get_http_referer());
